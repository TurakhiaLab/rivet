  ...
  function track(d) {
    ...
    if (num_snps > 40) {
      container_width = (buffer_btw_bases * (num_snps - 1)) +
          (num_snps * fixed_square_dims) + track_x_position + right_side_buffer;
    ...
    container.append('text')
        ...
        .text('Single-nucleotide variation in the recombinant and its parents');

    document.getElementById('download_svg').style.visibility = 'visible';
    ...
    if (d['COLOR']['environment'] == 'production') {
      ...
      fetch('/get_overview', {
        ...
        res.json().then(data => {
          append_text(
              overview, 'Recombinant Node ID: ' + d['NODE_IDS']['Recomb']);
          append_text(
              overview,
      ...
      var overview_info_button = document.createElement('button');
      overview_info_button.setAttribute('class', 'btn btn-outline-primary');
      overview_info_button.setAttribute('id', 'info');
      const info_btn_text = document.createTextNode('More Info');
      ...
      if (overview_info_button) {
        var overview_info = overview_info_button.onclick = function() {
          var myOffcanvas = document.getElementById('off_canvas_right')
      ...
      let overview_usher_button = document.createElement('button');
      overview_usher_button.setAttribute('class', 'btn btn-outline-primary');
      overview_usher_button.setAttribute('id', 'view_usher');
      const usher_btn_text = document.createTextNode('View UShER Tree (Recomb)');
      overview_usher_button.appendChild(usher_btn_text);
      overview.appendChild(overview_usher_button);

      //TODO: Experimental, donor/acceptor sampling, refactor
      let overview_usher_button_donor = document.createElement('button');
      overview_usher_button_donor.setAttribute('class', 'btn btn-outline-primary');
      overview_usher_button_donor.setAttribute('id', 'view_usher_donor');
      const usher_btn_text_donor = document.createTextNode('View UShER Tree (Donor)');
      overview_usher_button_donor.appendChild(usher_btn_text_donor);
      overview.appendChild(overview_usher_button_donor);

      let overview_usher_button_acceptor = document.createElement('button');
      overview_usher_button_acceptor.setAttribute('class', 'btn btn-outline-primary');
      overview_usher_button_acceptor.setAttribute('id', 'view_usher_acceptor');
      const usher_btn_text_acceptor = document.createTextNode('View UShER Tree (Acceptor)');
      overview_usher_button_acceptor.appendChild(usher_btn_text_acceptor);
      overview.appendChild(overview_usher_button_acceptor);


      const tooltip = d3.select('#view_usher')
                          .append('div')
      ...
      if (overview_usher_button) {
        overview_usher_button.onclick = function() {
          view_usher_tree(d, "Recomb");
        };
      }
      if (overview_usher_button_donor) {
        overview_usher_button_donor.onclick = function() {
          view_usher_tree(d, "Donor");
        };
      }
      if (overview_usher_button_acceptor) {
        overview_usher_button_acceptor.onclick = function() {
          view_usher_tree(d, "Acceptor");
        };
      }
...
*/
      ...
      if (private_table_select.hidden) {
        ...
        overview_tree_button.setAttribute('class', 'btn btn-outline-primary');
      ...
      let overview_mutations_button = document.createElement('button');
      overview_mutations_button.setAttribute(
          'class', 'btn btn-outline-primary');
      overview_mutations_button.setAttribute('id', 'show_mutations');
      const mutations_btn_text =
          document.createTextNode('Show Amino Acid Mutations');
      overview_mutations_button.appendChild(mutations_btn_text);
      ...
      if (overview_mutations_button) {
        overview_mutations_button.onclick = function() {
          ...
          if ($('#show_mutations').hasClass('active')) {
            active = false;
            $('#show_mutations').removeClass('active');
          } else {
            active = true;
            $('#show_mutations').addClass('active');
          }
          if (active) {
            let nt_data = get_amino_acid_mutations(
                d['NODE_IDS']['Recomb'], container, y_position, snp_positions,
    ...
    if (previous_button) {
      var previous_element = previous_button.onclick = function() {
    ...
    if (download_svg) {
      ...
      download_svg.onclick = function() {
        download_snv_plot(inner, d['NODE_IDS']['Recomb']);
      };
    }
    ...
    var reference_track_list = add_reference_track(
        container, polygon_buffer, y_position, square_dims, reference_snps,
    ...
    var trio_list =
        add_trio_track(container, y_position, square_dims, d, container_width);
